...
def get_losses(pred, gt):
    
    mae = abs(pred - gt) / gt if gt != 0 else abs(pred - gt)
    mse = (pred - gt) ** 2 / gt if gt != 0 else (pred - gt) ** 2
...
@torch.no_grad()
# def evaluate_crowd_no_overlap(model, data_loader, device, vis_dir="visualization"):
def evaluate_crowd_no_overlap(model, data_loader, device, vis_dir=None):
    ...
    for samples, targets in data_loader:
        ...
        # print(outputs['pred_logits'].shape) # (1, 20480, 2)
        # print(outputs['pred_points'].shape) # (1, 20480, 2)
        ...
        threshold = 0.5
        # print(outputs_points.shape) # (20480, 2)
        # print(outputs_scores.shape) # (20480)
        # print(gt_cnt) 
        ...
        mae, mse = get_losses(predict_cnt, gt_cnt)
