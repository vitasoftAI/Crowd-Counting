...
class Decoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        P4_x = self.P4_1(C4)
        P4_x = P5_upsampled_x + self.P4_upsampled(P4_x)
        P4_x = self.P4_2(P4_x)
...
class P2PNet(nn.Module):
    def __init__(self, backbone, row=2, line=2):
        ...
        self.fpn = Decoder(40, 384, 1536)

    def forward(self, samples: NestedTensor):
        # get the backbone features
        
        features = []
        for name, block in self.backbone.named_children():
            if name in ["global_pool", "classifier"]: continue
            if name == "conv_stem": out = block(samples)
            else: out = block(out)
            features.append(out)
        
        # forward the feature pyramid
        features_fpn = self.fpn([features[1], features[2], features[4]])
...
def build(args, device, training):
    # treats persons as a single class
    num_classes = 1

    # backbone = build_backbone(args)
    backbone = timm.create_model("efficientnet_b3a", pretrained = True).to(device)
