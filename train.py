...
def get_args_parser():
    ...
    parser.add_argument('--data_root', default='/home/ubuntu/workspace/bekhzod/DM-Count/datasets/jhu_crowd_v2.0',
                        help='path where the dataset is')
    
    parser.add_argument('--output_dir', default='log',
                        help='path where to save, empty for no saving')
    parser.add_argument('--checkpoints_dir', default='saved_models_test',
...
def main(args):
    ...
    run_log_name = os.path.join(args.output_dir, f'run_log_{args.dataset_file}_{args.backbone}.txt')
    ...
    model, criterion = build_model(args, device, training=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not os.path.isdir(f"{args.checkpoints_dir}_{args.dataset_file}_{args.backbone}"):
            os.makedirs(f"{args.checkpoints_dir}_{args.dataset_file}_{args.backbone}", exist_ok=True)
            print("Folder to save models is created!")
        checkpoint_latest_path = os.path.join(f"{args.checkpoints_dir}_{args.dataset_file}_{args.backbone}", 'latest.pth')
        torch.save({
        ...
        if epoch % args.eval_freq == 0 and epoch != 0:
            ...
            with torch.no_grad():
                ...
                if abs(np.min(mae) - result[0]) < 0.01:
                    checkpoint_best_path = os.path.join(f"{args.checkpoints_dir}_{args.dataset_file}_{args.backbone}", 'best_mae.pth')
                    torch.save({
                        'model': model_without_ddp.state_dict(),
